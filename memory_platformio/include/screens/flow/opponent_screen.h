...
class OpponentScreen : public Screen {
 ...
 public:
  ...
  void on_enter() override {
    ...
    // Fresh battle round — drop tutorial placeholders / stale player times.
    ...
    if (leaderboard_size > 0) {
      ...
      if (opp.games_count > 0) {
        ...
        opponent_targets.capture(current_round);
        return;
      }
    }
  ...
  void draw() override {
    ...
    display.drawTextCentered(_opponent_name, SCREEN_WIDTH_HALF, SCREEN_HEIGHT_HALF, WHITE);
    display.setFont(&Panell_Regular12pt7b);
    display.drawTextCentered("your oponent:", SCREEN_WIDTH_HALF, SCREEN_HEIGHT_HALF - 32, WHITE);
    display.drawTextCentered("rank #" + String(_opponent_rank), SCREEN_WIDTH_HALF, SCREEN_HEIGHT_HALF + 32, WHITE);
  }
};
